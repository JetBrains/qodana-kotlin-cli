...
| `toolchain/node`     | Node + Yarn for JS/TS analysis (in-place — no `FROM`, extends the current stage)                    | jvm, python, go, php                          |
...
| `toolchain/eslint`   | global ESLint (pinned in `eslint/package.json`, renovate-npm-tracked) for JS/TS analysis (in-place) | js, go, php (later ruby)                      |
...
| `dist`               | download + GPG/sha256-verify the IDE dist (`provision-dist`), then `verify-dist-layout`             | jvm, android, python(-community), js, go, php |
...
`conda-toolchain`, php to `php-toolchain`; jvm keeps `base`), `privileged` builds `FROM ${PRIVILEGED_BASE_STAGE}` (cdnet sets it
