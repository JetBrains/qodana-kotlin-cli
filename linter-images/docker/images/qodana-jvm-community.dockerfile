...
INCLUDE_ARGS images/qodana-jvm-community.env
...
# JBR font-manager native libs (QD-15265): IDEA's bundled JBR dlopens libfreetype.so.6 when it renders
# the Maven/Gradle sync build view during project-model configuration; absent -> NoClassDefFoundError
...
# and qodana-rust (QD-15111); IDEA is JBR-only (no .NET backend), so only the fonts are needed. Installed
# as root (apt), then USER drops back so the shipped image still scans unprivileged.
ARG QODANA_UID
ARG QODANA_GID
USER 0
RUN <<-EOT
