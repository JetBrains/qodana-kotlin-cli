...
# complete JDK for the Gradle daemon. Runs inside the qodana-android-community linter container, where
# ANDROID_HOME=/opt/android-sdk and Corretto 11/17 live under /opt/java (QD-15022, QD-14924).
set -euxo pipefail

: "${ANDROID_HOME:?ANDROID_HOME must be exported by the qodana-android-community image}"

sdkmanager="$ANDROID_HOME/cmdline-tools/latest/bin/sdkmanager"
[[ -x $sdkmanager ]] || { echo "android-community-app-agp bootstrap: sdkmanager not found at $sdkmanager" >&2; exit 1; }
...
[[ -x $corretto17/bin/java ]] || { echo "android-community-app-agp bootstrap: Corretto 17 not found at $corretto17" >&2; exit 1; }
