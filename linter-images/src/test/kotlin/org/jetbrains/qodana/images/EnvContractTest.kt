...
class EnvContractTest {
    ...
    fun `qodana-ruby env has exactly the jvm key set plus DIST_BASE_STAGE`() {
        // RubyMine-on-DHI-ruby-base: the ruby base ships ruby/gem/bundle pre-baked but NO node, so ruby
        // layers the node toolchain (NODE_MAJOR) + the in-place eslint pin + the in-place gem-cache
        // redirect (lib/toolchain/ruby.dockerfile) — jvm's key set. Ruby scans shell out to sudo to
        // install project gems (source PRIVILEGED=true), so ruby INCLUDEs lib/privileged.dockerfile and
        // its dist FROMs the privileged stage: DIST_BASE_STAGE=privileged is an .env KEY (base.dockerfile
        // does NOT default DIST_BASE_STAGE, so the INCLUDE_ARGS value survives — the android/php
        // convention). PRIVILEGED_BASE_STAGE=base is the dual: a COMPOSE BUILD ARG only (base.dockerfile
        // defaults it to clang-toolchain, which would clobber an .env value), so it must NOT be an .env
        // key. The ruby base does NOT occupy uid 1000 (empirically: /etc/passwd has only
        // root/nonroot/_apt/nobody, like the golang/php base), so — unlike qodana-js — ruby keeps the
        // default uid 1000 and sets NO uid keys/build args. The eslint pin lives in
        // lib/toolchain/eslint/package.json (renovate-tracked), NOT in the .env.
        ...
        assertEquals(
            expected,
            env.keys,
            "ruby must be jvm's key set (dist + node toolchain) plus DIST_BASE_STAGE",
        )
        assertEquals(
            "privileged",
            env["DIST_BASE_STAGE"],
        ...
        assertTrue(
            ...
            "ruby PRIVILEGED_BASE_STAGE is a compose build arg (base.dockerfile clobbers an .env value), not an .env key",
        )
        assertTrue(
            "QODANA_UID" !in env && "QODANA_GID" !in env,
    ...
    fun `ruby variants share the same dist pins and slug, differing only in base image`() {
        // The IDE dist is ruby-runtime-agnostic (the android precedent), so all 3 variants share the SAME
        // RM dist (QD_LINTER_SLUG=qodana-ruby) + identical dist pins, differing ONLY in QD_BASE_IMAGE.
        ...
        for (variant in listOf("qodana-ruby-3.2", "qodana-ruby-3.4")) {
            ...
            assertTrue(
                primary["QD_BASE_IMAGE"] != v["QD_BASE_IMAGE"],
                "$variant must use its OWN ruby base digest",
            )
