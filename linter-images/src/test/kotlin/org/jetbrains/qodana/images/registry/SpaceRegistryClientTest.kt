...
class SpaceRegistryClientTest {
    ...
    private fun wireSharedChildChain(created: String) {
        fake.on("GET", "$api/manifests/sha256:aaaa") { ex ->
            FakeOciServer.respond(
                ex, 200,
                """{"schemaVersion":2,"config":{"digest":"sha256:cccc"}}""",
                "application/vnd.oci.image.manifest.v1+json",
            )
        }
        fake.on("GET", "$api/blobs/sha256:cccc") { ex ->
            FakeOciServer.respond(ex, 200, """{"created":"$created"}""", "application/vnd.oci.image.config.v1+json")
        }
    }
    ...
    fun `listTags resolves pushed from the linux-amd64 config blob's created field`() {
        ...
        wireIndex("t1")
        fake.on("GET", "$api/tags/list") { ex -> FakeOciServer.respond(ex, 200, """{"tags":["t1"]}""", "application/json") }
    ...
    fun `listTags falls back to a safe sentinel and warns when no linux-amd64 manifest exists`() {
        fake.on("GET", "$api/manifests/t1") { ex ->
            FakeOciServer.respond(
                ex, 200,
                """{"schemaVersion":2,"manifests":[{"digest":"sha256:bad","platform":{"architecture":"unknown","os":"unknown"}}]}""",
                "application/vnd.oci.image.index.v1+json",
            )
        }
        fake.on("GET", "$api/tags/list") { ex -> FakeOciServer.respond(ex, 200, """{"tags":["t1"]}""", "application/json") }
        ...
        val tags = client.listTags(repo)
        assertEquals(Instant.MAX, tags.single().pushed, "an unresolvable tag must not abort the repo or look prunable-by-age")
    }
    ...
    fun `listTags follows Link-header pagination across 3 pages, with no page cap`() {
        ...
        fake.on("GET", "$api/tags/list") { ex ->
            when (ex.requestURI.query) {
                "last=b" -> FakeOciServer.respond(ex, 200, """{"tags":["c"]}""", "application/json")
                "last=a" -> {
                    ex.responseHeaders.add("Link", """<$api/tags/list?last=b>; rel="next"""")
                    FakeOciServer.respond(ex, 200, """{"tags":["b"]}""", "application/json")
                }
                else -> {
                    ex.responseHeaders.add("Link", """<$api/tags/list?last=a>; rel="next"""")
                    FakeOciServer.respond(ex, 200, """{"tags":["a"]}""", "application/json")
                }
            }
        }
    ...
    fun `listTags fails loud on a Link header that signals a next page in an unrecognized format`() {
        fake.on("GET", "$api/tags/list") { ex ->
            ...
            FakeOciServer.respond(ex, 200, """{"tags":["t1"]}""", "application/json")
        }
    ...
    fun `listTags fails loud when the tags-list endpoint returns a non-200`() {
        fake.on("GET", "$api/tags/list") { ex -> ex.sendResponseHeaders(500, -1); ex.close() }
    ...
    fun `resolveDigest returns the Docker-Content-Digest header for the tag's index manifest`() {
        fake.on("GET", "$api/manifests/t1") { ex ->
            ...
            FakeOciServer.respond(ex, 200, """{"schemaVersion":2,"manifests":[]}""", "application/vnd.oci.image.index.v1+json")
        }
    ...
    fun `resolveDigest returns null when the tag has no index manifest`() {
        fake.on("GET", "$api/manifests/t1") { ex -> ex.sendResponseHeaders(404, -1); ex.close() }
    ...
    fun `resolveDigest fails loud on a non-200-non-404 response`() {
        fake.on("GET", "$api/manifests/t1") { ex -> ex.sendResponseHeaders(500, -1); ex.close() }
    ...
    fun `resolveDigest requests a pull-only scoped token`() {
        fake.on("GET", "$api/manifests/t1") { ex ->
            ex.responseHeaders.add("Docker-Content-Digest", "sha256:deadbeef")
            FakeOciServer.respond(ex, 200, """{"manifests":[]}""", "application/vnd.oci.image.index.v1+json")
        }
    ...
    fun `deleteByDigest treats a 404 as success since a digest-delete is idempotent ensure-absent`() {
        fake.on("DELETE", "$api/manifests/sha256:gone") { ex -> ex.sendResponseHeaders(404, -1); ex.close() }
    ...
    fun `deleteByDigest requests a pull,delete scoped token, not merely pull`() {
        fake.on("DELETE", "$api/manifests/sha256:deadbeef") { ex -> ex.sendResponseHeaders(202, -1); ex.close() }
    ...
    fun `pushedAt follows a redirect when fetching the config blob`() {
        ...
        fake.on("GET", "/signed-blob") { ex ->
            FakeOciServer.respond(ex, 200, """{"created":"2026-04-04T00:00:00Z"}""", "application/vnd.oci.image.config.v1+json")
        }
    ...
    fun `repeated calls reuse a cached token instead of re-authenticating every time`() {
        ...
        wireIndex("t1")
        wireIndex("t2")
        fake.on("GET", "$api/tags/list") { ex -> FakeOciServer.respond(ex, 200, """{"tags":["t1","t2"]}""", "application/json") }

        client.listTags(repo)

        assertEquals(1, fake.tokenRequests.size, "one repo, one action (pull) -> one token request, not one per HTTP call")
    }
    ...
    fun `resolveDigest reuses the digest pushedAt already learned from the same manifest, no extra fetch`() {
        ...
        fake.on("GET", "$api/manifests/t1") { ex ->
            ...
            FakeOciServer.respond(
                ex, 200,
                """{"schemaVersion":2,"manifests":[{"digest":"sha256:aaaa","platform":{"architecture":"amd64","os":"linux"}}]}""",
                "application/vnd.oci.image.index.v1+json",
            )
        }
        ...
        assertEquals(1, indexFetches, "resolveDigest must reuse the digest listTags's pushedAt already read, not re-fetch")
    }
    ...
    fun `a fresh listTags drops a stale cached digest so a re-verify observes true current state`() {
        ...
        assertNull(client.resolveDigest(repo, "t1"), "resolveDigest must re-fetch after a fresh listTags, not serve the stale cached digest")
    }

    @Test
    fun `a token request failure surfaces a clear error`() {
        fake.on("GET", "$api/tags/list") { ex -> FakeOciServer.respond(ex, 200, """{"tags":[]}""", "application/json") }
