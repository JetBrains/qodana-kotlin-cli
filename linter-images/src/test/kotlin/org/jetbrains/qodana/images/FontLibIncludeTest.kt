 ...
 * every image that INCLUDEs it. qodana-cpp keeps its own inline block, so it is covered by CppImageTest,
 * not here. EnvContractTest cannot see this (no `.env` key), so this reads the Dockerfile sources directly.
 */
