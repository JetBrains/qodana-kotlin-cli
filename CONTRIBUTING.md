...
Both the scan smoke test and the CI `e2e` job (CLI workflow) pin `jetbrains/qodana-jvm-community` via `qodana-jvm-community-tag` in [`gradle/libs.versions.toml`](gradle/libs.versions.toml). When bumping the tag:
