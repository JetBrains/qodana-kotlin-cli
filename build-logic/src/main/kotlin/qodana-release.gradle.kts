...
//          Archive contains `qodana` (or `qodana.exe` on windows) at mode 0755. The amd64 → x86_64
//          mapping ONLY applies to the cli archive name — kept for Go-pipeline parity.
...
//          containing `<module>_<version>_<os>_<arch>.exe`. The windows `.zip` (vs a bare `.exe`) is
//          a deliberate divergence from `.goreleaser.yaml` kept for asset-name stability; coordinate
//          with Go-pipeline consumers before changing it.
//
// Tasks:
...
// Windows-tool zip equivalent of releaseZip. Tool kind on linux/darwin still ships a raw binary
// (releaseBinary writes directly to releaseDir for that path); on windows we wrap the renamed .exe
// in a zip with the same naming stem the raw .exe used to have. The .zip (vs a bare .exe) is kept
// for asset-name stability with the Go-pipeline contract (QD-14925).
val releaseToolZip = tasks.register<Zip>("releaseToolZip") {
    group = "release"
    description = "Zip archive of a qodana-tool windows binary."
    onlyIf {
...
// assembleRelease lifecycle. Tool on Windows ships a zip (asset-name parity); everything else
