...
class SystemProcessRunnerTest {
    ...
    @Test
    fun `run command with non-zero exit`() =
        runTest {
            val result =
                runner.run(
                    ProcessSpec(
    ...
    @Test
    fun `run with working directory`() =
        runTest {
            val result =
                runner.run(
                    ProcessSpec(
    ...
    @Test
    fun `run with environment variables`() =
        runTest {
            val result =
                runner.run(
                    ProcessSpec(
    ...
    @Test
    fun `start preserves carriage returns in output chunks`() =
        runTest(timeout = 10.seconds) {
            val process =
                runner.start(
                    ProcessSpec(
    ...
    @Test
    fun `run command that does not exist returns non-zero or throws`() =
        runTest {
            try {
                val result =
                    runner.run(
    ...
    @Test
    fun `run returns timeout placeholder on timeout`() =
        runTest {
            val result =
                runner.run(
                    ProcessSpec(
    ...
    @Test
    fun `start returns timeout placeholder on timeout`() =
        runTest {
            val process =
                runner.start(
                    ProcessSpec(
