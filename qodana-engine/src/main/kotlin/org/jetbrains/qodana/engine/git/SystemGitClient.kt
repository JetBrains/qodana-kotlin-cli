...
class SystemGitClient(
    private val processRunner: ProcessRunner,
) : GitClient {
    override suspend fun revParse(
        workDir: Path,
        ref: String,
    ): Result<String> = git(workDir, "rev-parse", ref).map { it.trim() }

    override suspend fun checkout(
        workDir: Path,
        ref: String,
    ): Result<Unit> = git(workDir, "checkout", ref).map { }
    ...
    override suspend fun branch(workDir: Path): Result<String> = git(workDir, "branch", "--show-current").map { it.trim() }

    override suspend fun remoteUrl(workDir: Path): Result<String> = git(workDir, "remote", "get-url", "origin").map { it.trim() }
    ...
    override suspend fun isGitRepo(workDir: Path): Boolean = git(workDir, "rev-parse", "--is-inside-work-tree").isSuccess

    override suspend fun currentBranch(workDir: Path): Result<String> = git(workDir, "rev-parse", "--abbrev-ref", "HEAD").map { it.trim() }

    override suspend fun currentRevision(workDir: Path): Result<String> = git(workDir, "rev-parse", "HEAD").map { it.trim() }
