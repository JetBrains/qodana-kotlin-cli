...
object EapCheck {
    ...
    sealed interface EapResult {
        data class Valid(val expirationDate: Instant) : EapResult
        data class Expired(val expirationDate: Instant) : EapResult
        data class InvalidDate(val dateStr: String) : EapResult
     ...
     */
    fun checkEap(isEap: Boolean, buildDateStr: String, now: Instant = Instant.now()): EapResult {
