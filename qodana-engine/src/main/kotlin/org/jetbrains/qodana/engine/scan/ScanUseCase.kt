...
class ScanUseCase(
    ...
    suspend fun run(context: ScanContext): Int {
        val yaml = context.yaml ?: EffectiveConfig.load(
    ...
    private fun handleSpecialExitCode(exitCode: Int, context: ScanContext): Int? {
    ...
    private suspend fun exportBitBucket(context: ScanContext) {
        ...
        val token = System.getenv("QD_BITBUCKET_TOKEN")
            ?: System.getenv("QODANA_TOKEN")
            ?: return
        val bitbucketUrl = System.getenv("QD_BITBUCKET_URL")
    ...
    private suspend fun runScoped(context: ScanContext, scenario: RunScenario.Scoped): Int {
        ...
        val endRef = context.runtime.diffEnd?.takeIf { it.isNotBlank() }
            ?: git.currentRevision(repositoryRoot).getOrElse {
                terminal.error("Cannot detect current revision: ${it.message}")
                return 1
            }
        val scopeFile = createScopeFile(context, scenario.targetBranch, endRef).getOrElse {
        ...
        return try {
            ...
            val firstStageContext = scopedStageContext(
                base = context,
                script = "scoped:${scopeFile}",
                resultsDir = firstResultsDir,
                skipFixes = true,
                clearBaseline = true,
                additionalPropertyFlags = listOf(
            ...
            val secondStageContext = scopedStageContext(
                base = context,
                script = "scoped:${scopeFile}",
    ...
    private suspend fun runReverseScoped(context: ScanContext, scenario: RunScenario.ReverseScoped): Int {
        val git = gitClient ?: return runSingleAnalysis(context)
        val repositoryRoot = context.paths.repositoryRoot
        val endRef = context.runtime.diffEnd?.takeIf { it.isNotBlank() }
            ?: git.currentRevision(repositoryRoot).getOrElse {
        ...
        return try {
            ...
            val newStageContext = scopedStageContext(
                base = context,
            ...
            val oldStageContext = scopedStageContext(
                base = context,
            ...
            if (context.runtime.applyFixes || context.runtime.cleanup) {
                ...
                val fixesStageContext = scopedStageContext(
                    base = context,
    ...
    private suspend fun createScopeFile(context: ScanContext, startRef: String, endRef: String): Result<Path?> {
        val git = gitClient ?: return Result.success(null)
        return git.diff(context.paths.repositoryRoot, startRef, endRef).map { rawDiff ->
            val repoRoot = context.paths.repositoryRoot.toAbsolutePath().normalize()
            val projectDir = context.paths.projectDir.toAbsolutePath().normalize()
            val changedFiles = rawDiff.lineSequence()
                .map { it.trim() }
            ...
            val payload = mapOf(
                "files" to changedFiles.map { path ->
                    mapOf(
    ...
    private fun copyCoverageFromNewStage(sourceCoverageDir: Path, targetResultsDir: Path) {
    ...
    private fun copyStageResults(fromDir: Path, toDir: Path) {
    ...
    private suspend fun runFullHistory(context: ScanContext): Int {
        ...
        val revisions = git.log(repositoryRoot, "%H")
            .getOrDefault("")
            .lines()
            .map { it.trim() }
            .filter { it.isNotBlank() }
        ...
        val startHash = context.runtime.diffStart
        val filtered = if (!startHash.isNullOrBlank()) {
        ...
        for ((index, revision) in filtered.withIndex()) {
            ...
            val contextForIteration = context.copy(
                ci = context.ci.copy(
