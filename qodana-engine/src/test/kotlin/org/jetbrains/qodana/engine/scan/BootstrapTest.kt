...
class BootstrapTest {
    ...
    @Test
    fun `empty command returns 0`(@TempDir dir: Path) {
        assertEquals(0, Bootstrap.execute("", dir))
    }

    @Test
    fun `blank command returns 0`(@TempDir dir: Path) {
        assertEquals(0, Bootstrap.execute("  ", dir))
    }

    @Test
    fun `echo command succeeds`(@TempDir dir: Path) {
        val exitCode = Bootstrap.execute("echo hello", dir)
        assertEquals(0, exitCode)
    }

    @Test
    fun `capture stdout`(@TempDir dir: Path) {
    ...
    @Test
    fun `capture stderr`(@TempDir dir: Path) {
    ...
    @Test
    fun `empty capture returns empty`(@TempDir dir: Path) {
