...
class ContainerScanTest {
    @Test
    fun `scan pulls image, creates, starts, waits, removes`() =
        runTest {
    ...
    @Test
    fun `scan skips pull when skipPull is true`() =
        runTest {
            val ops = mutableListOf<String>()
            val engine = FakeContainerEngine(ops, exitCode = 0)
            val terminal = FakeTerminal()
    ...
    @Test
    fun `scan returns non-zero exit code`() =
        runTest {
            val ops = mutableListOf<String>()
    ...
    @Test
    fun `scan preserves carriage return log chunks`() =
        runTest {
            val ops = mutableListOf<String>()
            val logs =
                flowOf(
    ...
    @Test
    fun `scan uses spinner for pull in interactive terminal`() =
        runTest {
            val ops = mutableListOf<String>()
            val engine =
                FakeContainerEngine(
    ...
    @Test
    fun `scan prints pull phase line in non interactive terminal`() =
        runTest {
            val ops = mutableListOf<String>()
            val engine =
                FakeContainerEngine(
    ...
    @Test
    fun `scan updates interactive stage progress from logs`() =
        runTest {
            val ops = mutableListOf<String>()
            val logs =
                flowOf(
    ...
    @Test
    fun `scan mounts project, results, report, cache dirs`() =
        runTest {
    ...
    @Test
    fun `scan passes QODANA_TOKEN as env var`() =
        runTest {
            val engine = CapturingContainerEngine()
            val terminal = FakeTerminal()
    ...
    @Test
    fun `dotnet image gets SYS_PTRACE capability`() =
        runTest {
            val engine = CapturingContainerEngine()
            val terminal = FakeTerminal()
    ...
    @Test
    fun `non-dotnet image does not get SYS_PTRACE`() =
        runTest {
            val engine = CapturingContainerEngine()
            val terminal = FakeTerminal()
    ...
    @Test
    fun `scan passes additional volumes`() =
        runTest {
            val engine = CapturingContainerEngine()
            val terminal = FakeTerminal()

            val ctx =
                testContext(image = "test:latest").copy(
                    docker =
                        DockerOptions(
                            image = "test:latest",
