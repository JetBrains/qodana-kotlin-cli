...
class FailureThresholdsTest {
    ...
    fun `multiple severity thresholds`() {
        val yaml = QodanaYaml(
            failureConditions = YamlFailureConditions(
                severityThresholds = YamlSeverityThresholds(
                    any = 1, critical = 2, high = 3, moderate = 4, low = 5, info = 6
                )
            )
        )
        val thresholds = FailureThresholds.getFailureThresholds(yaml)
        val args = FailureThresholds.thresholdsToArgs(thresholds).sorted().joinToString("") { " $it" }
        assertEquals(" --threshold-any=1 --threshold-critical=2 --threshold-high=3 --threshold-info=6 --threshold-low=5 --threshold-moderate=4", args)
    }
    ...
    fun `failThreshold fills any when severity thresholds lack any`() {
        val yaml = QodanaYaml(
            ...
            failureConditions = YamlFailureConditions(
                severityThresholds = YamlSeverityThresholds(
                    critical = 2, high = 3, moderate = 4, low = 5, info = 6
                )
            )
        )
        val thresholds = FailureThresholds.getFailureThresholds(yaml)
        val args = FailureThresholds.thresholdsToArgs(thresholds).sorted().joinToString("") { " $it" }
        assertEquals(" --threshold-any=123 --threshold-critical=2 --threshold-high=3 --threshold-info=6 --threshold-low=5 --threshold-moderate=4", args)
    }
