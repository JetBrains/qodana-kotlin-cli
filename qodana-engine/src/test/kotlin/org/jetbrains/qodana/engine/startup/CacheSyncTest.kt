...
class CacheSyncTest {
    ...
    @Test
    fun `syncIdeaCache copies idea dir from cache to project`(
        @TempDir tmpDir: Path,
    ) {
    ...
    @Test
    fun `syncIdeaCache does not overwrite existing idea dir`(
        @TempDir tmpDir: Path,
    ) {
        val cacheDir = tmpDir.resolve("cache")
        val projectDir = tmpDir.resolve("project")
    ...
    @Test
    fun `syncIdeaCache no cache no problem`(
        @TempDir tmpDir: Path,
    ) {
    ...
    @Test
    fun `syncIdeaCache skips symlinks`(
        @TempDir tmpDir: Path,
    ) {
        val cacheDir = tmpDir.resolve("cache")
        val projectDir = tmpDir.resolve("project")
    ...
    @Test
    fun `syncConfigCache copies jdk table from cache when not present`(
        @TempDir tmpDir: Path,
    ) {
    ...
    @Test
    fun `syncConfigCache does not overwrite existing jdk table`(
        @TempDir tmpDir: Path,
    ) {
        val confDir = tmpDir.resolve("conf")
        val cacheDir = tmpDir.resolve("cache")
    ...
    @Test
    fun `syncConfigCache stores jdk table to cache`(
        @TempDir tmpDir: Path,
    ) {
        val confDir = tmpDir.resolve("conf")
        val cacheDir = tmpDir.resolve("cache")
    ...
    @Test
    fun `writeFileIfNew creates file when not present`(
        @TempDir tmpDir: Path,
    ) {
    ...
    @Test
    fun `writeFileIfNew does not overwrite existing file`(
        @TempDir tmpDir: Path,
    ) {
