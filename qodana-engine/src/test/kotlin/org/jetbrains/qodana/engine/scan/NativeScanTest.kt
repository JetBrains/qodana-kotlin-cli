...
class NativeScanTest {
    ...
    private fun expectedIdeScript(ideDir: Path = Path.of("/opt/ide")): String =
        if (isMac) {
            ideDir.resolve("MacOS/goland").toString()
        } else {
            ideDir.resolve("bin/goland").toString()
        }

    @Test
    fun `run throws when ideDir is null`() =
        runTest {
    ...
    @Test
    fun `run uses SARIF exit code when present`() =
        runTest {
            val fs = RecordingFileSystem()
    ...
    @Test
    fun `non-zero process exit takes precedence over SARIF exit code`() =
        runTest {
            val fs = RecordingFileSystem()
    ...
    @Test
    fun `invalid SARIF exit code is normalized to 1`() =
        runTest {
            val fs = RecordingFileSystem()
    ...
    @Test
    fun `run falls back to process exit code when no SARIF`() =
        runTest {
            val fs = RecordingFileSystem()
            setupIdeFiles(fs)
    ...
    @Test
    fun `run writes idea properties before execution`() =
        runTest {
            val fs = RecordingFileSystem()
            setupIdeFiles(fs)
    ...
    @Test
    fun `run resolves native IDE binary`() =
        runTest {
            val fs = RecordingFileSystem()
            setupIdeFiles(fs)
    ...
    @Test
    fun `run adds qodana subcommand`() =
        runTest {
            val fs = RecordingFileSystem()
            setupIdeFiles(fs)
    ...
    @Test
    fun `run forwards native process output to terminal`() =
        runTest {
            val fs = RecordingFileSystem()
            setupIdeFiles(fs)
    ...
    @Test
    fun `run preserves carriage return chunks in native output`() =
        runTest {
            val fs = RecordingFileSystem()
            setupIdeFiles(fs)
            val terminal = NativeOutputTerminal()
    ...
    @Test
    fun `run throws when IDE binary not found`() =
        runTest {
            val fs = RecordingFileSystem()
    ...
    @Test
    fun `install plugins uses dedicated install vm options`() =
        runTest {
            val fs = RecordingFileSystem()
            setupIdeFiles(fs)
            fs.fileContents[Path.of("/results/qodana.sarif.json")] = """{"exitCode": 0}"""
            ...
            assertTrue(
                processRunner.runSpecs
                    .single()
                    .env.values
                    .contains(installVmOptionsPath.toString()),
            )
        }

    // QD-15107: PyCharm/RubyMine/CLion/GoLand do NOT bundle org.intellij.qodana; it ships in the dist's
    // custom-plugins/ dir. The scan must load it via -Dplugin.path (else QodanaApplicationStarter is
    // never registered → "Application cannot start in a headless mode") and apply the curated
    // custom-plugins/disabled_plugins.txt via -Ddisabled.plugins.file.path (which disables HtmlTools so
    // CLion's bundled Angular cascade-disables instead of fatally erroring). Mirrors the Go CLI.
    @Test
    fun `scan loads the dist custom-plugins via -Dplugin path and disabled-plugins file`() =
        runTest {
            val fs = RecordingFileSystem()
    ...
    @Test
    fun `scan omits custom-plugin options when the dist bundles qodana (no custom-plugins dir)`() =
        runTest {
            val fs = RecordingFileSystem()
            setupIdeFiles(fs)
            fs.fileContents[Path.of("/results/qodana.sarif.json")] = """{"exitCode": 0}"""
            ...
            val vm = fs.writtenFiles[Path.of("/cache/idea-config/idea64.vmoptions")]
            assertTrue(vm != null && !vm.contains("-Dplugin.path") && !vm.contains("disabled.plugins.file.path"))
        }
...
private class RecordingFileSystem : FileSystem {
    ...
    override fun walk(
        root: Path,
        glob: String?,
    ) = existingFiles.filter { it != root && it.startsWith(root) }.asSequence()
